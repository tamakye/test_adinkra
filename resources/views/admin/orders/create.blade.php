...
<div class="content">
    <div class="container-fluid">
        <form id="orderForm" class="orderForm" action="{{ route('orders.create') }}" method="post">
            @csrf
            <input type="hidden" name="order_number" value=" {{ get_order_number() }} " id="order_number">
        ...
        <div class="row">
            <div class="col-md-9">
                        ...
                        <table id="orderProductTable" class="table col-sm-12 table-condensed cf">
                            ...
                            <tbody>
                                <form class="basketForm">
                                    ...
                                    @foreach(Cart::session(get_order_number())->getContent() as $product)
                                    <tr class="">
                                        <input type="hidden" class="items" name="items[]" value="{{  $product->id }}">
                                        <td class="d-flex align-items-center pl-0 pb-0">
                                            <span>
                                                <button type="button" class="btn btn-icon remove_item" data-item={{ $product->id }}>
                                        ...
                                        <td class="">
                                            <input class="col-4 basket_quantity form-control m-auto" autocomplete="off" min="1" name="quantity[]" type="number" value="{{ $product->quantity }}">

                                        </td>
                                        <td class="text-right">
                                            £{{ number_format($product->getPriceSum(), 2) }}
                                        </td>
                                    </tr>
                                    @endforeach
                                    ...
                                    <tr>
                                        <td colspan="4">
                                            <button type="button" class="btn border float-right update_product"><i class="fas fa-sync"></i> update</button>
                        ...
                        <div class="col-md-4 offset-8 bg-light">
                          <ul class="list-group reload-total" >
                              <li class="list-group-item d-flex justify-content-between">
                                  ...
                                  <span class="font-weight-bolder">£{{ number_format(Cart::session(get_order_number())->getSubTotal(), 2) }}</span>
                              </li>

                              <li class="list-group-item d-flex justify-content-between">
                                <span class="font-weight-bold">Vat:</span>
                                <span class="font-weight-bolder">£{{  number_format(getTaxValue(session('tax'), get_order_number()), 2) }} 
                                    @if(getTaxValue(session('tax'), get_order_number()) > 0)
                                    <br><small class="float-right text-muted cusor remove_tax" data-toggle="tooltip" data-placement="bottom" title="Remove tax"><i class="fas fa-trash"></i></small>
                                    @endif
                                </span>
                            </li>
                            ...
                            <li class="list-group-item d-flex justify-content-between">
                                ...
                                <span class="font-weight-bolder">£{{  number_format(Cart::session(get_order_number())->getTotal(), 2) }} </span>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="card-footer bg-white border-top d-flex justify-content-end">
                    <button type="button" class="btn btn-outline-primary add_product">Add product(s)</button>
                    <button type="button" class="btn btn-outline-primary ml-1 add_shipping">Add shpping</button>
                    <button type="button" class="btn btn-outline-primary ml-1"  data-toggle="modal" data-target="#taxModal">Add tax</button>
                </div>
            </div>
        </div>
